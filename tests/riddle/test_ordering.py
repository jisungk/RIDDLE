...
Unit test(s) for the `ordering.py` module.
...
class TestSummary:
    ...
    def summary(self):
        ...
        list_freq_ordering = [[['A', 300.0],['B', 200.0],['C', 100.0],['D', 0.0]], \
        ...
        return OrderingSummary(od)

    def test_sort(self, summary):
        sorted_summary = copy.deepcopy(summary) # not sorted this line; will be
        sorted_summary.sort()
    ...
    def test_save(self, summary):
        ...
        list_contrib_ordering = [' > '.join([stringify_pair(p) for p in o]) \
            for o in summary.od['contrib_ordering']]
        list_freq_ordering = [' > '.join([stringify_pair(p) for p in o]) \
            for o in summary.od['freq_ordering']]

        for row_idx in range(1, len(summary.od.values()[0])):
            assert lines[row_idx] == '\t'.join([list_feat[row_idx - 1], \
                list_descript[row_idx - 1], list_contrib_ordering[row_idx - 1],
...
def test_compute_orderings():
    # orderings by contribution score
    sums_contribs = np.array([[1, 2, 3, 4], [4, 3, 2, 1], [-5, -5, -5, -5]])
    contrib_orderings = compute_orderings(sums_contribs)
    expected_contrib_orderings = [[(1, 4), (0, 1), (2, -5)], \
        [(1, 3), (0, 2), (2, -5)], [(0, 3), (1, 2), (2, -5)], \
        [(0, 4), (1, 1), (2, -5)]]
    ...
    class_feat_freq_table = np.array([[5, 3, 4], [6, 1, 3], [7, 0, 2], 
        [9, 4, 1]])
    freq_orderings = compute_orderings(class_feat_freq_table)
    expected_freq_orderings = [[(3, 9), (2, 7), (1, 6), (0, 5)], \
        [(3, 4), (0, 3), (1, 1), (2, 0)], [(0, 4), (1, 3), (2, 2), (3, 1)]]
    ...
    prop_orderings = compute_orderings(class_feat_prop_table)
    expected_prop_orderings = [[(2, 7/9.), (3, 9/14.), (1, 6/10.), (0, 5/12.)], \
        [(3, 4/14.), (0, 3/12.), (1, 1/10.), (2, 0.)], \
        [(0, 4/12.), (1, 3/10.), (2, 2/9.), (3, 1/14.)]]
    assert np.all(prop_orderings == expected_prop_orderings)

def test_compute_prop_table():
    class_feat_freq_table = np.array([[5, 3, 4], [6, 1, 3], [7, 0, 2], 
        [9, 4, 1]])
    expected_class_feat_prop_table = [[5/12., 3/12., 4/12.], \
        [6/10., 1/10., 3/10.], [7/9., 0/9., 2/9.], [9/14., 4/14., 1/14.]]
    class_feat_prop_table = compute_prop_table(class_feat_freq_table)
    assert np.all(class_feat_prop_table == expected_class_feat_prop_table)

def test_decode_orderings():
    raw_orderings = [[[0, 0.5], [1, 1.5], [2, 2.5], [3, 3.5]], \
        [[3, 3], [2, 2], [1, 1], [0, 0]]]
    idx_class_dict = {0: 'A', 1:'B', 2:'C', 3:'D'}

    decoded_orderings = decode_orderings(raw_orderings, idx_class_dict)
